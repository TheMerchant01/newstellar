...
const TransferInSchema = z.object({
  type: z.literal(TransactionType.TRANSFER_IN),
  amount: z.number().min(0.01, "Amount must be greater than 0"),
  currency: z.string().min(1, "Currency is required"),
  accountType: z.enum(["checkingAccount", "investmentAccount"]),
  transferInDetails: z.object({
    senderName: z.string().min(1, "Sender name is required"),
    senderAccountNumber: z.string().min(1, "Sender account number is required"),
    senderBankName: z.string().min(1, "Sender bank name is required"),
...
type TransferInFormData = z.infer<typeof TransferInSchema>;

// Union type for form data
type TransactionFormData =
  | CryptoDepositFormData
  | ChequeDepositFormData
  | TransferFormData
  | TransferInFormData;
...
export function AddTransactionDialog({
  ...
  const transferInForm = useForm<TransferInFormData>({
    ...
    if (open) {
      ...
      const fetchCurrencies = async () => {
        try {
          ...
          const currencyData: ICurrencyWithId[] =
          ...
          if (defaultCurrency) {
            ...
            cryptoForm.setValue(
              ...
              defaultCurrency.name === "BTC" ? "Bitcoin" : "Ethereum"
            );
            cryptoForm.setValue(
              "cryptoDetails.walletAddress",
              defaultCurrency.walletAddress
            );
          }
        } catch (error) {
  ...
  }, [open, cryptoForm, chequeForm, transferForm, transferInForm]);
  ...
  const cleanup = useCallback(() => {
    ...
    transferForm.reset();
    transferInForm.reset();
  }, [cryptoForm, chequeForm, transferForm, transferInForm]);
  ...
  const handleCurrencyChange = (value: string) => {
    ...
    transferInForm.setValue("currency", value);
    setSelectedAddress(selectedCurrency?.walletAddress || "");
    if (selectedCurrency) {
      cryptoForm.setValue(
        "cryptoDetails.network",
        selectedCurrency.name === "BTC" ? "Bitcoin" : "Ethereum"
      );
      cryptoForm.setValue(
        "cryptoDetails.walletAddress",
  ...
  const handleCryptoSubmit: SubmitHandler<CryptoDepositFormData> = async (
    data
  ) => {
    if (!proofOfPayment) {
  ...
  const handleChequeSubmit: SubmitHandler<ChequeDepositFormData> = async (
    data
  ) => {
  ...
  const handleTransferSubmit: SubmitHandler<TransferFormData> = async (
    data
  ) => {
  ...
  const handleTransferInSubmit: SubmitHandler<TransferInFormData> = async (
    data
  ) => {
    setFormData(data);
    setShowConfirmation(true);
  ...
  const confirmTransaction = async () => {
    ...
    try {
      const response = await api.post(
        `/admin/users/${retailerId}/transactions`,
        {
          userId: customer._id,
          ...formData,
  ...
  const currentBalance =
    customer[formData?.accountType || "checkingAccount"].balance;
      ...
      <div className="relative z-50 w-full max-w-[700px] max-h-[90vh] overflow-y-auto bg-white rounded-lg shadow-lg">
        <div className="flex items-center justify-between p-6 border-b">
          ...
          <div className="flex items-center gap-2">
            ...
            <span>
              {customer.firstName} {customer.lastName} (ID: {customer._id})
            </span>
          </div>
        </div>
        <div className="p-6">
          {!showConfirmation && !showSuccess ? (
            <div className="space-y-6">
              ...
              >
                      ...
                      <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            ...
                            <Select
                              onValueChange={(value) =>
                                cryptoForm.setValue("accountType", value as any)
                              }
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          ...
                          <Alert>
                            ...
                            <AlertDescription>
                              Send cryptocurrency to the address above. The
                              deposit will be processed once confirmed on the
                              blockchain.
                            </AlertDescription>
                          </Alert>
                        </CardContent>
                ...
                <TabsContent value="cheque" className="space-y-6">
                  <form
                    onSubmit={chequeForm.handleSubmit(handleChequeSubmit)}
                    className="space-y-6"
                  >
                    <Card>
                      <CardHeader>
                      ...
                      <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            ...
                            <Select
                              onValueChange={(value) =>
                                chequeForm.setValue("accountType", value as any)
                              }
                              defaultValue="checkingAccount"
                            >
                              <SelectTrigger>
                      ...
                      <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            ...
                            />
                            {chequeForm.formState.errors.chequeDetails
                              ?.chequeNumber && (
                              <p className="text-red-500 text-sm flex items-center">
                                <AlertCircle className="h-3 w-3 mr-1" />
                                {
                                  chequeForm.formState.errors.chequeDetails
                                    .chequeNumber.message
                                }
                              </p>
                            )}
                          </div>
                          <div className="space-y-2">
                            ...
                            <Input
                              id="date"
                              type="date"
                              {...chequeForm.register("chequeDetails.date")}
                            />
                            {chequeForm.formState.errors.chequeDetails
                              ?.date && (
                              <p className="text-red-500 text-sm flex items-center">
                    ...
                    <Card>
                      ...
                      <CardContent className="space-y-6">
                        ...
                        <Alert>
                          <AlertCircle className="h-4 w-4" />
                          <AlertDescription>
                            Please ensure both sides of the cheque are clearly
                            visible and all information is legible.
                          </AlertDescription>
                        </Alert>
                      </CardContent>
                    </Card>
                      ...
                      <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            ...
                            <Input
                              id="accountName"
                              {...transferForm.register(
                                "transferDetails.accountName"
                              )}
                              placeholder="John Doe"
                            />
                            {transferForm.formState.errors.transferDetails
                              ?.accountName && (
                              <p className="text-red-500 text-sm flex items-center">
                                <AlertCircle className="h-3 w-3 mr-1" />
                                {
                ...
                <TabsContent value="transferIn" className="space-y-6">
                  <form
                    onSubmit={transferInForm.handleSubmit(
                      handleTransferInSubmit
                    )}
                    className="space-y-6"
                  >
                    <Card>
                      ...
                      <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            <Label htmlFor="accountType">To Account</Label>
                            <Select
                              onValueChange={(value) =>
                                transferInForm.setValue(
                                  "accountType",
                                  value as any
                                )
                              }
                            ...
                            {transferInForm.formState.errors.accountType && (
                              <p className="text-red-500 text-sm flex items-center">
                                <AlertCircle className="h-3 w-3 mr-1" />
                                {
                                  transferInForm.formState.errors.accountType
                          ...
                          <div className="space-y-2">
                            ...
                            {transferInForm.formState.errors.currency && (
                              <p className="text-red-500 text-sm flex items-center">
                                <AlertCircle className="h-3 w-3 mr-1" />
                                {
                                  transferInForm.formState.errors.currency
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="relative">
                            <Input
                              ...
                              placeholder="0.00"
                              className="pl-8"
                              {...transferInForm.register("amount", {
                                valueAsNumber: true,
                              })}
                            />
                            <DollarSign className="absolute left-2 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-500" />
                          </div>
                          {transferInForm.formState.errors.amount && (
                            <p className="text-red-500 text-sm flex items-center">
                              <AlertCircle className="h-3 w-3 mr-1" />
                              {transferInForm.formState.errors.amount.message}
                            </p>
                          )}
                        </div>
                        <div className="space-y-2">
                      ...
                      <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            ...
                            <Input
                              id="senderName"
                              {...transferInForm.register(
                                "transferInDetails.senderName"
                              )}
                              placeholder="John Doe"
                            />
                            {transferInForm.formState.errors.transferInDetails
                              ?.senderName && (
                              <p className="text-red-500 text-sm flex items-center">
                                <AlertCircle className="h-3 w-3 mr-1" />
                                {
                                  transferInForm.formState.errors
                                    .transferInDetails.senderName.message
                                }
                              </p>
                            )}
                          </div>
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              {...transferInForm.register(
                                "transferInDetails.senderAccountNumber"
                              )}
                              placeholder="1234567890"
                            />
                            {transferInForm.formState.errors.transferInDetails
                        ...
                        <div className="space-y-2">
                          ...
                          <Input
                            id="senderBankName"
                            {...transferInForm.register(
                              "transferInDetails.senderBankName"
                            )}
                            placeholder="Bank Name"
                          />
                          {transferInForm.formState.errors.transferInDetails
                            ...
                            <p className="text-red-500 text-sm flex items-center">
                              ...
                              {
                                transferInForm.formState.errors
                                  .transferInDetails.senderBankName.message
                              }
                            </p>
                          )}
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            ...
                            <Select
                              onValueChange={(value) =>
                                transferInForm.setValue(
                                  "transferInDetails.senderCountry",
                                  value
                                )
                              }
                              defaultValue=""
                            ...
                            {transferInForm.formState.errors.transferInDetails
                              ...
                              <p className="text-red-500 text-sm flex items-center">
                                ...
                                {
                                  transferInForm.formState.errors
                                    .transferInDetails.senderCountry.message
                                }
                              </p>
                            )}
                          </div>
                          <div className="space-y-2">
                            ...
                            <Input
                              id="referenceNumber"
                              {...transferInForm.register(
                                "transferInDetails.referenceNumber"
                              )}
                            ...
                            {transferInForm.formState.errors.transferInDetails
                              ...
                              <p className="text-red-500 text-sm flex items-center">
                                ...
                                {
                                  transferInForm.formState.errors
                                    .transferInDetails.referenceNumber.message
                                }
                              </p>
                            )}
                          </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <Textarea
                            id="description"
                            {...transferInForm.register(
                              "transferInDetails.description"
                            )}
                            placeholder="e.g., Payment received from..."
                            className="resize-none"
                            rows={2}
                          />
                        </div>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">
                        {
                          accounts.find((a) => a.id === formData?.accountType)
                            ?.name
                        }
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="font-semibold">
                        {
                          currencies.find((c) => c._id === formData?.currency)
                            ?.name
                        }
                      </p>
                    </div>
                        ...
                        <div className="grid grid-cols-1 gap-3">
                          <div>
                            ...
                            <p className="font-mono text-sm">
                              {formData.cryptoDetails.network}
                            </p>
                          </div>
                          <div>
                            <Label className="text-sm text-gray-600">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            ...
                            <p className="font-semibold">
                              {formData.chequeDetails.chequeNumber}
                            </p>
                          </div>
                          <div>
                            <Label className="text-sm text-gray-600">
                            ...
                            <p className="font-semibold">
                              {format(
                                new Date(formData.chequeDetails.date),
                                "PPP"
                              )}
                            </p>
                          </div>
                          <div className="col-span-2">
                            ...
                            <p className="text-sm">
                              {formData.chequeDetails.description}
                            </p>
                          </div>
                          <div>
                            <Label className="text-sm text-gray-600">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            ...
                            <p className="font-semibold">
                              {formData.transferDetails.accountName}
                            </p>
                          </div>
                          <div>
                            <Label className="text-sm text-gray-600">
                            ...
                            <p className="font-semibold">
                              {formData.transferDetails.accountNumber}
                            </p>
                          </div>
                          <div>
                            <Label className="text-sm text-gray-600">
                            ...
                            <p className="font-semibold">
                              {formData.transferDetails.bankName}
                            </p>
                          </div>
                          <div>
                            <Label className="text-sm text-gray-600">
                            ...
                            <p className="font-semibold">
                              {
                                countries.find(
                                  (c) =>
                                    c.code === formData.transferDetails.country
                                )?.name
                              }
                            </p>
                          </div>
                          <div>
                            ...
                            <p className="font-semibold">
                              {formData.transferDetails.swiftCode}
                            </p>
                          </div>
                          ...
                          <div className="col-span-2">
                            ...
                            <p className="font-semibold">
                              {formData.transferDetails.bankAddress}
                            </p>
                          </div>
                  ...
                  {formData?.type === TransactionType.TRANSFER_IN &&
                    "transferInDetails" in formData && (
                      <div className="space-y-3">
                        <h4 className="font-semibold">Transfer In Details</h4>
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            <Label className="text-sm text-gray-600">
                              Sender Name
                            ...
                            <p className="font-semibold">
                              {formData.transferInDetails.senderName}
                            </p>
                          </div>
                          <div>
                            <Label className="text-sm text-gray-600">
                            ...
                            <p className="font-semibold">
                              {formData.transferInDetails.senderAccountNumber}
                            </p>
                          </div>
                          <div>
                            <Label className="text-sm text-gray-600">
                            ...
                            <p className="font-semibold">
                              {formData.transferInDetails.senderBankName}
                            </p>
                          </div>
                          <div>
                            <Label className="text-sm text-gray-600">
                            ...
                            <p className="font-semibold">
                              {
                                countries.find(
                                  (c) =>
                                    c.code ===
                                    formData.transferInDetails.senderCountry
                                )?.name
                              }
                            </p>
                          </div>
                          <div>
                            ...
                            <p className="font-semibold">
                              {formData.transferInDetails.referenceNumber}
                            </p>
                          </div>
                          {formData.transferInDetails.description && (
                            <div className="col-span-2">
                              <Label className="text-sm text-gray-600">
                  ...
                  {newBalance < 0 && (
                    <Alert variant="destructive">
                      <AlertCircle className="h-4 w-4" />
                      <AlertDescription>
                        Warning: This transaction will result in a negative
                        balance.
                      </AlertDescription>
                    </Alert>
                  )}
            ...
            <div className="flex flex-col items-center justify-center py-12 space-y-6">
              ...
              <p className="text-gray-600 text-center max-w-md">
                The transaction has been processed and will be reflected in the
                customer's account shortly.
              </p>
              <Button
                onClick={() => {
                  resetForm();
